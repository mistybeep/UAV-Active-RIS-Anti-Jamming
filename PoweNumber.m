...
PtNum = db2pow([30:2:40] - 30); % 40dBm
alphaMaxNum = 5:10;
PropPowerNumSimuRes = zeros(length(PtNum), length(alphaMaxNum));
...
for n = 1 : length(PtNum)
    Pt = PtNum(n);
    for m = 1 : length(alphaMaxNum)

        alphaMax = alphaMaxNum(m);

        thetaVecCurrent = alphaMax * thetaVecCurrentDataSet{6} / 10;
    
        fprintf(' ------- Current Transmit Power: %d W and RIS Amplitude: %d ------- \n', Pt, alphaMax);
        ...
        toc
        PropPowerNumSimuRes(n, m) = mean(cell2mat(resPropFinal));
    end
end

save("PowerRes.mat", "PropPowerNumSimuRes");
