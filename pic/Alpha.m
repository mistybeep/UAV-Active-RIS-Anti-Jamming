...
Prop(end) = PropRISNumSimuRes(end,7);
MM(end) = MMRISNumSimuRes(end,7);
SDR(end) = SDRRISNumSimuRes(end,7);
Static(end) = PropStaticRISNumSimuRes(end,7);
Passive = PassiveRISNumSimuRes(end,7)*ones(10, 1);
% 创建x轴数据
iter = 1:10;
...
% 设置坐标轴标签和其他属性
xlabel('Active RIS Amplification Factor, $\beta$ (dB)', 'Interpreter', 'latex', ...
...
axesPosition = [0.3 0.59 0.24 0.18]; 
